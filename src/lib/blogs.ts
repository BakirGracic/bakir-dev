export const blogs = [
  {
    title: "Counter-Strike 2 Dedicated Server in 2026",
    published: "2025-12-23",
    slug: "cs2-dedicated-server-2026",
    description:
      "Learn how to host your own Counter-Strike 2 dedicated server with Hetzner in simple steps to enjoy the game your way with friends outside the network",
  },
  {
    title: "Bosnian Car Registration Plates with C++: Structure and Variations",
    published: "2024-07-12",
    slug: "bosnian-registration-plates-cpp",
    description:
      "Explore the structure and variations of Bosnian automobile registration plates, learn cool facts, and discover a unique mini C++ project that lists all possible plate variations",
  },
];
