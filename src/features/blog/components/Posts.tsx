...
export default function Posts({ limit = -1 }) {
	...
	return blogPosts.map((post) => (
		<Link key={post.slug} href={`/blog/${post.slug}`} tabIndex={0}>
			<article>
				<div className='mb-5 flex flex-col md:flex-row'>
					<p className='darkerText mb-2 flex min-w-full flex-row gap-1 text-sm md:mb-0 md:min-w-[130px] md:flex-col'>
