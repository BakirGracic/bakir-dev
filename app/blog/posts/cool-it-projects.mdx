---
title: 'Cool IT Projects'
publishedAt: '2024-06-01'
summary: 'Sometimes building cool and small projects help in 2 ways. Apart from boosting your experience, projects can become a useful tool in your arsenal. Read more!'
...
Typically, your ISP assigns a DNS server to your router/device. While this works fine for most consumers, some prefer custom, editable, and controllable solutions.
...
Set AdGuard as the primary DNS server in your router settings so all network devices will use it for DNS. Remember, if your server can't resolve DNS queries, your devices won't function properly, even if they show an internet connection. Also, you can use AdGuard as a DHCP server!

## VPN Server

While you have your DNS server running, why not install VPN software on it? VPN providers often charge unaffordable monthly fees, but you can have a free VPN server with unlimited and faster bandwidth. The only drawback is that you can use only one location (exit node) per server/software.
...
Install it via Docker using [WG Easy](https://github.com/wg-easy/wg-easy) to get a web control interface easily (since the software is CLI-oriented out of the box).
...
My use cases include movie hosting and streaming (for home use only!), file backups, file sharing, music hosting and streaming, and running a DNS and VPN server to emulate accessing the internet from my home network. All of this is accessible outside my network, anywhere in the world by additionally setting port forwarding on in my router settings and havig a static IPv4 for convinience.
